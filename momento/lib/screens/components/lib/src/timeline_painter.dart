...
class _TimelinePainterCenter extends _TimelinePainter {
  _TimelinePainterCenter({iconSize, properties, isFirst, isLast, iconBackground})
      : super(
            iconSize: iconSize,
            properties: properties,
            isFirst: isFirst,
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    final Offset topLineEnd = configuration.size
        .center(Offset(0.0, offset.dy - iconSize / 2 - 2 * TimelineBoxDecoration.LINE_GAP));
    final Offset bottomLineStart = configuration.size
        .center(Offset(0.0, offset.dy + iconSize / 2 + 2 * TimelineBoxDecoration.LINE_GAP));
    ...
    final Offset offsetCenter = configuration.size.center(Offset(0.0, offset.dy));
    canvas.drawCircle(offsetCenter, iconSize / 2 + TimelineBoxDecoration.LINE_GAP, circlePaint);
  }
}
...
class _TimelinePainterLeft extends _TimelinePainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    if (iconSize == TimelineBoxDecoration.DEFAULT_DOT_SIZE)
      iconMargin = TimelineBoxDecoration.DEFAULT_ICON_SIZE / 2 + 2 * TimelineBoxDecoration.LINE_GAP;
    else
    ...
    final Offset centerTop =
        configuration.size.centerLeft(Offset(leftOffset.dx, leftOffset.dy - iconMargin));
...
class _TimelinePainterRight extends _TimelinePainter {
  ...
  void paint(Canvas canvas, Offset offset, ImageConfiguration configuration) {
    ...
    final Offset centerTop =
        configuration.size.centerRight(Offset(rightOffset.dx, rightOffset.dy - iconMargin));
    final Offset centerBottom =
        configuration.size.centerRight(Offset(rightOffset.dx, rightOffset.dy + iconMargin));
